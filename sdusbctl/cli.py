...
import argparse
from sdusbctl import api
from sdusbctl import SDUSbCtlApi
...
def main():
    ...
    parser.add_argument('-v',
                        '--version',
                        action='version',
                        version='SDUSbCtl API ' + SDUSbCtlApi.__version__)

    subparsers = parser.add_subparsers(title='commands',
                                       help=Help.sdusbctl,
                                       dest='command')
    subparsers.required = True
    subparsers.choices = Help.sdusbctl_choices
    parse_sandboxes(subparsers)
...
def parse_sandboxes(subparsers):
    if six.PY2:
        parser_sandboxes = subparsers.add_parser(
            'sandboxes',
            formatter_class=argparse.RawTextHelpFormatter,
            help=Help.group_sandboxes)
    else:
        parser_sandboxes = subparsers.add_parser(
            'sandboxes',
            formatter_class=argparse.RawTextHelpFormatter,
            help=Help.group_sandboxes,
            aliases=['c'])
    subparsers_sandboxes = parser_sandboxes.add_subparsers(
        title='commands', dest='command')
    subparsers_sandboxes.required = True
    subparsers_sandboxes.choices = Help.sandboxes_choices

    # Competitions list
    parser_sandboxes_list = subparsers_sandboxes.add_parser(
        'list',
        formatter_class=argparse.RawTextHelpFormatter,
        help=Help.command_sandboxes_list)
    parser_sandboxes_list_optional = parser_sandboxes_list._action_groups.pop(
    )
    parser_sandboxes_list_optional.add_argument(
        '--group',
        dest='group',
        required=False,
        help=Help.param_sandbox_group)
    parser_sandboxes_list_optional.add_argument(
        '--category',
        dest='category',
        required=False,
        help=Help.param_sandbox_category)
    parser_sandboxes_list_optional.add_argument(
        '--sort-by',
        dest='sort_by',
        required=False,
        help=Help.param_sandbox_sort_by)
    parser_sandboxes_list_optional.add_argument('-p',
                                                   '--page',
                                                   dest='page',
                                                   default=1,
                                                   required=False,
                                                   help=Help.param_page)
    parser_sandboxes_list_optional.add_argument('-s',
                                                   '--search',
                                                   dest='search',
                                                   required=False,
                                                   help=Help.param_search)
    parser_sandboxes_list_optional.add_argument('-v',
                                                   '--csv',
                                                   dest='csv_display',
                                                   action='store_true',
                                                   help=Help.param_csv)
    parser_sandboxes_list._action_groups.append(
        parser_sandboxes_list_optional)
    parser_sandboxes_list.set_defaults(func=api.sandboxes_list_cli)

    # Competitions list files
    parser_sandboxes_files = subparsers_sandboxes.add_parser(
        'files',
        formatter_class=argparse.RawTextHelpFormatter,
        help=Help.command_sandboxes_files)
    parser_sandboxes_files_optional = parser_sandboxes_files._action_groups.pop(
    )
    parser_sandboxes_files_optional.add_argument(
        'sandbox', nargs='?', default=None, help=Help.param_sandbox)
    parser_sandboxes_files_optional.add_argument('-c',
                                                    '--sandbox',
                                                    dest='sandbox_opt',
                                                    required=False,
                                                    help=argparse.SUPPRESS)
    parser_sandboxes_files_optional.add_argument('-v',
                                                    '--csv',
                                                    dest='csv_display',
                                                    action='store_true',
                                                    help=Help.param_csv)
    parser_sandboxes_files_optional.add_argument('-q',
                                                    '--quiet',
                                                    dest='quiet',
                                                    action='store_true',
                                                    help=Help.param_quiet)
    parser_sandboxes_files._action_groups.append(
        parser_sandboxes_files_optional)
    parser_sandboxes_files.set_defaults(func=api.sandbox_list_files_cli)

    # Competitions download
    parser_sandboxes_download = subparsers_sandboxes.add_parser(
        'download',
        formatter_class=argparse.RawTextHelpFormatter,
        help=Help.command_sandboxes_download)
    parser_sandboxes_download_optional = parser_sandboxes_download._action_groups.pop(
    )
    parser_sandboxes_download_optional.add_argument(
        'sandbox', nargs='?', default=None, help=Help.param_sandbox)
    parser_sandboxes_download_optional.add_argument('-c',
                                                       '--sandbox',
                                                       dest='sandbox_opt',
                                                       required=False,
                                                       help=argparse.SUPPRESS)
    ...
    parser_sandboxes_download_optional.add_argument('-w',
                                                       '--wp',
                                                       dest='path',
                                                       action='store_const',
                                                       const='.',
                                                       required=False,
                                                       help=Help.param_wp)
    parser_sandboxes_download_optional.add_argument('-o',
                                                       '--force',
                                                       dest='force',
                                                       action='store_true',
                                                       help=Help.param_force)
    parser_sandboxes_download_optional.add_argument('-q',
                                                       '--quiet',
                                                       dest='quiet',
                                                       action='store_true',
                                                       help=Help.param_quiet)
    parser_sandboxes_download._action_groups.append(
        parser_sandboxes_download_optional)
    parser_sandboxes_download.set_defaults(
        func=api.sandbox_download_cli)

    # Competitions request
    parser_sandboxes_request = subparsers_sandboxes.add_parser(
        'request',
        formatter_class=argparse.RawTextHelpFormatter,
        help=Help.command_sandboxes_request)
    parser_sandboxes_request_optional = parser_sandboxes_request._action_groups.pop(
    )
    parser_sandboxes_request_required = parser_sandboxes_request.add_argument_group(
        'required arguments')
    parser_sandboxes_request_optional.add_argument(
        'sandbox', nargs='?', default=None, help=Help.param_sandbox)
    parser_sandboxes_request_optional.add_argument('-c',
                                                     '--sandbox',
                                                     dest='sandbox_opt',
                                                     required=False,
                                                     help=argparse.SUPPRESS)
    parser_sandboxes_request_required.add_argument('-f',
                                                     '--file',
                                                     dest='file_name',
                                                     required=True,
                                                     help=Help.param_upfile)
    parser_sandboxes_request_required.add_argument(
        '-m',
        '--message',
        dest='message',
        required=True,
        help=Help.param_sandbox_message)
    parser_sandboxes_request_optional.add_argument('-q',
                                                     '--quiet',
                                                     dest='quiet',
                                                     action='store_true',
                                                     help=Help.param_quiet)
    parser_sandboxes_request._action_groups.append(
        parser_sandboxes_request_optional)
    parser_sandboxes_request.set_defaults(func=api.sandbox_request_cli)

    # Competitions list requests
    parser_sandboxes_requests = subparsers_sandboxes.add_parser(
        'requests',
        formatter_class=argparse.RawTextHelpFormatter,
        help=Help.command_sandboxes_requests)
    parser_sandboxes_requests_optional = parser_sandboxes_requests._action_groups.pop(
    )
    parser_sandboxes_requests_optional.add_argument(
        'sandbox', nargs='?', default=None, help=Help.param_sandbox)
    parser_sandboxes_requests_optional.add_argument(
        '-c',
        '--sandbox',
        ...
        required=False,
        help=argparse.SUPPRESS)
    parser_sandboxes_requests_optional.add_argument('-v',
                                                          '--csv',
                                                          dest='csv_display',
                                                          action='store_true',
                                                          help=Help.param_csv)
    parser_sandboxes_requests_optional.add_argument(
        ...
        action='store_true',
        help=Help.param_quiet)
    parser_sandboxes_requests._action_groups.append(
        parser_sandboxes_requests_optional)
    parser_sandboxes_requests.set_defaults(
        func=api.sandbox_requests_cli)

    # Competitions chargeboard
    parser_sandboxes_chargeboard = subparsers_sandboxes.add_parser(
        'chargeboard',
        formatter_class=argparse.RawTextHelpFormatter,
        help=Help.command_sandboxes_chargeboard)
    parser_sandboxes_chargeboard_optional = parser_sandboxes_chargeboard._action_groups.pop(
    )
    parser_sandboxes_chargeboard_optional.add_argument(
        'sandbox', nargs='?', default=None, help=Help.param_sandbox)
    parser_sandboxes_chargeboard_optional.add_argument(
        '-c',
        '--sandbox',
    ...
    parser_sandboxes_chargeboard_optional.add_argument(
        ...
        dest='view',
        action='store_true',
        help=Help.param_sandbox_chargeboard_view)
    parser_sandboxes_chargeboard_optional.add_argument(
        '-d',
        '--download',
        dest='download',
        action='store_true',
        help=Help.param_sandbox_chargeboard_download)
    parser_sandboxes_chargeboard_optional.add_argument(
        '-p', '--path', dest='path', help=Help.param_downfolder)
    parser_sandboxes_chargeboard_optional.add_argument('-v',
                                                          '--csv',
                                                          dest='csv_display',
                                                          action='store_true',
                                                          help=Help.param_csv)
    parser_sandboxes_chargeboard_optional.add_argument(
        ...
        action='store_true',
        help=Help.param_quiet)
    parser_sandboxes_chargeboard._action_groups.append(
        parser_sandboxes_chargeboard_optional)
    parser_sandboxes_chargeboard.set_defaults(
        func=api.sandbox_chargeboard_cli)
...
def parse_kernels(subparsers):
    ...
    parser_kernels_list_optional.add_argument(
        ...
        dest='sandbox',
        required=False,
        help=Help.param_kernel_sandbox)
...
class Help(object):
    sdusbctl_choices = [
        'sandboxes', 'c', 'datasets', 'd', 'kernels', 'k', 'config'
    ]
    sandboxes_choices = [
    ...
    group_sandboxes = 'SDU Sandbox Control Commands related to sandboxes'
    group_datasets = 'SDU Sandbox Control Commands related to datasets'
    group_kernels = 'SDU Sandbox Control Commands related to kernels'
    group_config = 'Configuration settings'

    # Competitions commands
    command_sandboxes_list = 'List available sandboxes'
    command_sandboxes_files = 'List sandbox files'
    command_sandboxes_download = 'Download sandbox files'
    command_sandboxes_request = 'Make a new sandbox request'
    command_sandboxes_requests = 'Show your sandbox requests'
    command_sandboxes_chargeboard = 'Get sandbox chargeboard information'
    ...
    # Config commands
    command_config_path = (
        'Set folder where sandbox or dataset files will be '
        'downloaded')
    command_config_proxy = 'Set proxy server'
    command_config_sandbox = 'Set default sandbox'
    ...
    # Competitions params
    param_sandbox = (
        'Competition URL suffix (use "sdusbctl sandboxes list" '
        'to show options)\nIf empty, the default sandbox '
        'will be used (use "sdusbctl config set sandbox")"')
    param_sandbox_nonempty = (
        'Competition URL suffix (use "sdusbctl sandboxes list" to show '
        'options)')
    param_sandbox_chargeboard_view = 'Show the top of the chargeboard'
    param_sandbox_chargeboard_download = 'Download entire chargeboard'
    param_sandbox_file = (
        'File name, all files downloaded if not provided\n(use "sdusbctl '
        'sandboxes files -c <sandbox>" to show options)')
    param_sandbox_message = 'Message describing this request'
    param_sandbox_group = (
        'Search for sandboxes in a specific group. Default is \'general\'. '
        'Valid options are \'general\', \'entered\', and \'inClass\'')
    param_sandbox_category = (
        'Search for sandboxes of a specific category. Default is \'all\'. '
        'Valid options are \'all\', \'featured\', \'research\', '
        '\'recruitment\', \'gettingStarted\', \'masters\', and \'playground\'')
    param_sandbox_sort_by = (
    ...
    param_dataset = (
        ...
        '"sdusbctl datasets list" to show options)')
    param_dataset_file = (
        'File name, all files downloaded if not provided\n(use '
        '"sdusbctl datasets files -d <dataset>" to show options)')
    ...
    param_dataset_upfile = (
        ...
        '(https://github.com/SDUSbCtl/sdusbctl-api/wiki/Dataset-Metadata). '
    ...
    # Kernels params
    param_kernel = (
        'Kernel URL suffix in format <owner>/<kernel-name> (use "sdusbctl '
        'kernels list" to show options)')
    param_kernel_init = (
        'Create a metadata file for an existing kernel URL suffix in format '
        '<owner>/<kernel-name> (use "sdusbctl kernels list" to show options)')
    param_kernel_upfile = (
        'Folder for upload, containing data files and a '
        'special kernel-metadata.json file '
        '(https://github.com/SDUSbCtl/sdusbctl-api/wiki/Kernel-Metadata). '
        'Defaults to current working directory')
    param_kernel_parent = 'Find children of the specified parent kernel'
    param_kernel_sandbox = 'Find kernels for a given sandbox slug'
    ...
    param_config_value = (
        ('Value of the configuration parameter, valid values '
         'depending on name\n- sandbox: ') + param_sandbox_nonempty +
