...
from sdusbctl.configuration import Configuration
from .sdusbctl_api import SDUSBCtlApi
...
from ..models.sdusbctl_models_extended import Sandbox
from ..models.sdusbctl_models_extended import Dataset
from ..models.sdusbctl_models_extended import DatasetNewResponse
from ..models.sdusbctl_models_extended import DatasetNewVersionResponse
from ..models.sdusbctl_models_extended import File
from ..models.sdusbctl_models_extended import FileUploadInfo
from ..models.sdusbctl_models_extended import Kernel
from ..models.sdusbctl_models_extended import KernelPushResponse
from ..models.sdusbctl_models_extended import LeaderboardEntry
from ..models.sdusbctl_models_extended import ListFilesResult
from ..models.sdusbctl_models_extended import Metadata
from ..models.sdusbctl_models_extended import Request
from ..models.sdusbctl_models_extended import RequestResult
...
class SDUSbCtlApi(SDUSbCtlApi):
    __version__ = '1.0.0'

    CONFIG_NAME_PROXY = 'proxy'
    CONFIG_NAME_COMPETITION = 'sandbox'
    CONFIG_NAME_PATH = 'path'
    CONFIG_NAME_USER = 'username'
    CONFIG_NAME_KEY = 'key'
    CONFIG_NAME_SSL_CA_CERT = 'ssl_ca_cert'

    HEADER_API_VERSION = 'X-SDUSBCtl-ApiVersion'
    DATASET_METADATA_FILE = 'dataset-metadata.json'
    OLD_DATASET_METADATA_FILE = 'datapackage.json'
    KERNEL_METADATA_FILE = 'kernel-metadata.json'

    config_dir = os.environ.get('SDUSBCTL_CONFIG_DIR') or os.path.join(
        expanduser('~'), '.sdusbctl')
    if not os.path.exists(config_dir):
        os.makedirs(config_dir)

    config_file = 'sdusbctl.json'
    ...
    valid_sandbox_groups = ['general', 'entered', 'inClass']
    valid_sandbox_categories = [
        'all', 'featured', 'research', 'recruitment', 'gettingStarted',
        'masters', 'playground'
    ]
    valid_sandbox_sort_by = [
    ...
    # Hack for https://github.com/SDUSBCtl/sdusbctl-api/issues/22 / b/78194015
    if six.PY2:
        reload(sys)
    ...
    def authenticate(self):
        """authenticate the user with the SDUSBCtl API. This method will generate
           a configuration, first checking the environment for credential
           variables, and falling back to looking for the .sdusbctl/sdusbctl.json
           configuration file.
        """
    ...
    def read_config_environment(self, config_data=None, quiet=False):
        """read_config_environment is the second effort to get a username
           and key to authenticate to the SDUSBCtl API. The environment keys
           are equivalent to the sdusbctl.json file, but with "SDUSBCTL_" prefix
        ...
        # Add all variables that start with SDUSBCTL_ to config data

        if config_data is None:
            config_data = {}
        for key, val in os.environ.items():
            if key.startswith('SDUSBCTL_'):
                config_key = key.replace('SDUSBCTL_', '', 1).lower()
        ...
        try:
            ...
            if 'Proxy' in type(error).__name__:
                ...
                raise ValueError(
                    ...
                    'https://github.com/SDUSBCtl/sdusbctl-api#api-credentials ' +
                    'for instructions.')

    def read_config_file(self, config_data=None, quiet=False):
        """read_config_file is the first effort to get a username
           and key to authenticate to the SDUSBCtl API. Since we can get the
        ...
        if os.path.exists(self.config):
            ...
            try:
                if os.name != 'nt':
                    ...
                    if (permissions & 4) or (permissions & 32):
                        print(
                            'Warning: Your SDUSBCtl API key is readable by other '
            ...
            if not quiet:
                print('No SDUSBCtl API config file found, will use environment.')
    ...
    def sandboxes_list(self,
                          group=None,
                          category=None,
                          sort_by=None,
                          page=1,
                          search=None):
        """ make call to list sandboxes, format the response, and return
            a list of Sandbox instances
            ...
            sort_by: how to sort the result, see valid_sandbox_sort_by for options
            category: category to filter result to
            group: group to filter result to
        """
        if group and group not in self.valid_sandbox_groups:
            raise ValueError('Invalid group specified. Valid options are ' +
                             str(self.valid_sandbox_groups))

        if category and category not in self.valid_sandbox_categories:
            raise ValueError('Invalid category specified. Valid options are ' +
                             str(self.valid_sandbox_categories))

        if sort_by and sort_by not in self.valid_sandbox_sort_by:
            raise ValueError('Invalid sort_by specified. Valid options are ' +
                             str(self.valid_sandbox_sort_by))

        sandboxes_list_result = self.process_response(
            self.sandboxes_list_with_http_info(group=group or '',
                                                  category=category or '',
                                                  sort_by=sort_by or '',
                                                  page=page,
                                                  search=search or ''))
        return [Sandbox(c) for c in sandboxes_list_result]

    def sandboxes_list_cli(self,
                              group=None,
                              category=None,
                              sort_by=None,
                              page=1,
                              search=None,
                              csv_display=False):
        """ a wrapper for sandboxes_list for the client.
        ...
        """
        sandboxes = self.sandboxes_list(group=group,
                                              category=category,
                                              sort_by=sort_by,
                                              page=page,
                                              search=search)
        ...
        if sandboxes:
            if csv_display:
                self.print_csv(sandboxes, fields)
            else:
                self.print_table(sandboxes, fields)
        else:
            print('No sandboxes found')

    def sandbox_request(self, file_name, message, sandbox, quiet=False):
        """ request a sandbox!

            Parameters
            ==========
            file_name: the sandbox metadata file
            message: the request description
            sandbox: the sandbox name
            quiet: suppress verbose output (default is False)
        """
        if sandbox is None:
            sandbox = self.get_config_value(self.CONFIG_NAME_COMPETITION)
            if sandbox is not None and not quiet:
                print('Using sandbox: ' + sandbox)

        if sandbox is None:
            raise ValueError('No sandbox specified')
        else:
            url_result = self.process_response(
                self.sandboxes_requests_url_with_http_info(
                    id=sandbox,
            ...
            if 'isComplete' in url_result:
                ...
                if not success:
                    ...
                    return "Could not request to sandbox"

                upload_result_token = url_result['token']

            request_result = self.process_response(
                self.sandboxes_requests_request_with_http_info(
                    id=sandbox,
                    blob_file_tokens=upload_result_token,
                    request_description=message))
            return RequestResult(request_result)

    def sandbox_request_cli(self,
                               file_name,
                               message,
                               sandbox,
                               sandbox_opt=None,
                               quiet=False):
        """ request a sandbox using the client. Arguments are same as for
            sandbox_request, except for extra arguments provided here.
             Parameters
            ==========
            sandbox_opt: an alternative sandbox option provided by cli
        """
        sandbox = sandbox or sandbox_opt
        try:
            request_result = self.sandbox_request(file_name, message,
                                                    sandbox, quiet)
        except ApiException as e:
            if e.status == 404:
                print('Could not find sandbox - please verify that you '
                      'entered the correct sandbox ID and that the '
                      'sandbox is still accepting requests.')
                return None
            else:
                raise e
        return request_result

    def sandbox_requests(self, sandbox):
        """ get the list of Request for a particular sandbox

            Parameters
            ==========
            sandbox: the name of the sandbox
        """
        requests_result = self.process_response(
            self.sandboxes_requests_list_with_http_info(id=sandbox))
        return [Request(s) for s in requests_result]

    def sandbox_requests_cli(self,
                                    sandbox=None,
                                    sandbox_opt=None,
                                    csv_display=False,
                                    quiet=False):
        """ wrapper to sandbox_request, will return either json or csv
            to the user. Additional parameters are listed below, see
            sandbox_requests for rest.

            Parameters
            ==========
            sandbox: the name of the sandbox. If None, look to config
            sandbox_opt: an alternative sandbox option provided by cli
            csv_display: if True, print comma separated values
            quiet: suppress verbose output (default is False)
        """
        ...
        if sandbox is None:
            ...
            requests = self.sandbox_requests(sandbox)
            fields = [
            ...
            if requests:
                if csv_display:
                    self.print_csv(requests, fields)
                else:
                    self.print_table(requests, fields)
            else:
                print('No requests found')

    def sandbox_list_files(self, sandbox):
        """ list files for sandbox
             Parameters
            ==========
            sandbox: the name of the sandbox
        """
        sandbox_list_files_result = self.process_response(
            self.sandboxes_data_list_files_with_http_info(id=sandbox))
        return [File(f) for f in sandbox_list_files_result]

    def sandbox_list_files_cli(self,
                                   sandbox,
                                   sandbox_opt=None,
                                   csv_display=False,
                                   quiet=False):
        """ List files for a sandbox, if it exists
        ...
        if sandbox is None:
            ...
            files = self.sandbox_list_files(sandbox)
    ...
    def sandbox_download_file(self,
                                  sandbox,
                                  file_name,
                                  path=None,
                                  force=False,
                                  quiet=False):
        """ download a sandbox file to a designated location, or use
            a default location
        ...
        response = self.process_response(
            self.sandboxes_data_download_file_with_http_info(
                id=sandbox, file_name=file_name, _preload_content=False))
    ...
    def sandbox_download_files(self,
                                   sandbox,
                                   path=None,
                                   force=False,
                                   quiet=True):
        """ downloads all sandbox files.
        ...
        response = self.process_response(
            self.sandboxes_data_download_files_with_http_info(
                id=sandbox, _preload_content=False))
        url = response.retries.history[0].redirect_location.split('?')[0]
    ...
    def sandbox_download_cli(self,
                                 ...
                                 force=False,
                                 quiet=False):
        """ a wrapper to sandbox_download_files, but first will parse input
            from API client. Additional parameters are listed here, see
            sandbox_download for remaining.
        ...
        if sandbox is None:
            ...
            if file_name is None:
                self.sandbox_download_files(sandbox, path, force,
                                                quiet)
            else:
                self.sandbox_download_file(sandbox, file_name, path,
                                               force, quiet)

    def sandbox_chargeboard_download(self, sandbox, path, quiet=True):
        """ Download sandbox chargeboards
        ...
        """
        response = self.process_response(
            self.sandbox_download_chargeboard_with_http_info(
                sandbox, _preload_content=False))
        ...
        file_name = sandbox + '.zip'
        outfile = os.path.join(effective_path, file_name)
        self.download_file(response, outfile, quiet)

    def sandbox_chargeboard_view(self, sandbox):
        """ view a chargeboard based on a sandbox name

            Parameters
            ==========
            sandbox: the sandbox name to view leadboard for
        """
        result = self.process_response(
            self.sandbox_view_chargeboard_with_http_info(sandbox))
        return [LeaderboardEntry(e) for e in result['requests']]

    def sandbox_chargeboard_cli(self,
                                    sandbox,
                                    sandbox_opt=None,
                                    path=None,
                                    view=False,
                                    ...
                                    csv_display=False,
                                    quiet=False):
        """ a wrapper for sandbox_leaderbord_view that will print the
            ...
            download: if True, download the entire chargeboard
        ...
        if download:
            self.sandbox_chargeboard_download(sandbox, path, quiet)

        if view:
            results = self.sandbox_chargeboard_view(sandbox)
            fields = ['teamId', 'teamName', 'requestDate', 'score']
            if results:
                if csv_display:
            ...
            if unzip:
                try:
                    ...
                    raise ValueError(
                        ...
                        'www.github.com/sdusbctl/sdusbctl-api', e)
    ...
    def kernels_list(self,
        ...
        """ list kernels based on a set of search criteria
            ...
            sandbox: if defined, filter to this sandbox (default None)
    ...
    def kernels_push(self, folder):
        """ read the metadata file and kernel files from a notebook, validate
            both, and use Kernel API to push to SDUSBCtl if all is valid.
             Parameters
            ==========
            folder: the path of the folder
        """
        ...
        kernel_push_request = KernelPushRequest(
            ...
            sandbox_data_sources=self.get_or_default(
                meta_data, 'sandbox_sources', []),
    ...
    def kernels_push_cli(self, folder):
        ...
        if result is None:
            ...
            if result.invalidSandboxSources:
                print(
                    'The following are not valid sandbox sources and could '
                    'not be added to the kernel: ' +
                    str(result.invalidSandboxSources))
    ...
    def kernels_pull(self, kernel, path, metadata=False, quiet=True):
        ...
        if not os.path.isfile(effective_path):
            ...
            if file_name is None:
                print(
                    ...
                    'on the sdusbctl-api github issues' %
        ...
        if metadata:
            ...
            self.set_if_present(server_metadata, 'kernelDataSources', data,
                                'kernel_sources')
            self.set_if_present(server_metadata, 'sandboxDataSources',
                                data, 'sandbox_sources')
